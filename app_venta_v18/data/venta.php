...
function opcionesAfectacionVentaSelect($selected='10'){
    ...
    $selected = caf_extraer_codigo($selected, '10');
    $items = caf_catalogo_js_activo($idempresa, true);
    $html = '';
    ...
    foreach($items as $it){
        ...
        $desc = isset($it['descripcion']) ? trim((string)$it['descripcion']) : $cod;
        $descCorta = isset($it['descripcion_corta']) && trim((string)$it['descripcion_corta']) !== '' ? trim((string)$it['descripcion_corta']) : $desc;
        $codTrib = isset($it['cod_tributo']) ? trim((string)$it['cod_tributo']) : '1000';
        $pct = number_format((float)(isset($it['porcentaje_igv']) ? $it['porcentaje_igv'] : 0), 2, '.', '');
        $sel = ($selected === $cod) ? ' selected' : '';

        $html .= '<option value="'.$cod.'" data-codigo="'.$cod.'" data-descripcion="'.htmlspecialchars($desc, ENT_QUOTES, 'UTF-8').'" data-descripcion-corta="'.htmlspecialchars($descCorta, ENT_QUOTES, 'UTF-8').'" data-cod-tributo="'.$codTrib.'" data-porcentaje-igv="'.$pct.'"'.$sel.'>'.htmlspecialchars($descCorta, ENT_QUOTES, 'UTF-8').' ('.$cod.')</option>';
    }
...
function normalizarDetalleAfectacionVenta($idventa){
    ...
    $selectCols = array('iddetalle_venta','tipo','cod_afectacion_igv','cod_tributo','porc_igv','txtCANTIDAD_ARTICULO','precio');
    foreach(array('valor_unitario_ref','valor_total_ref','igv_ref','es_gratuito','es_bonificacion','es_publicidad','es_retiro') as $c){
        if(_table_has_column('detalle_venta',$c)){ $selectCols[]=$c; }
    }

    $rs = ejecutarConsulta("SELECT ".implode(',', $selectCols)." FROM detalle_venta WHERE idventa='".$idventa."'");
    if(!$rs){ return; }

    while($det = $rs->fetch_object()){
        $tipoLegacy = isset($det->tipo) ? trim((string)$det->tipo) : '';
        $codRaw = isset($det->cod_afectacion_igv) ? trim((string)$det->cod_afectacion_igv) : '';

        $cod = caf_extraer_codigo($codRaw, '');
        if($cod===''){ $cod = caf_tipo_to_codigo($tipoLegacy); }
        if($cod===''){ $cod = '10'; }

        $conf = caf_get_codigo_conf($cod, $idempresa);
        $cod = isset($conf['codigo']) && trim((string)$conf['codigo'])!=='' ? trim((string)$conf['codigo']) : $cod;
        $codTrib = isset($conf['cod_tributo']) ? trim((string)$conf['cod_tributo']) : '1000';
        if($codTrib===''){ $codTrib='1000'; }
        $porc = number_format((float)(isset($conf['porcentaje_igv']) ? $conf['porcentaje_igv'] : 0), 2, '.', '');

        $esNoOnerosa = ($codTrib==='9996') ? true : (isset($conf['es_onerosa']) ? ((int)$conf['es_onerosa']===0) : in_array($cod, array('11','12','13','14','15','16','21','31','32','33','34','35','36'), true));
        $requiereRef = ($codTrib==='9996') ? true : (isset($conf['requiere_valor_referencial']) ? ((int)$conf['requiere_valor_referencial']===1) : $esNoOnerosa);

        $cti = isset($det->txtCANTIDAD_ARTICULO) ? (float)$det->txtCANTIDAD_ARTICULO : 0;
        $precio = isset($det->precio) ? (float)$det->precio : 0;
        $vuRef = (isset($det->valor_unitario_ref) ? (float)$det->valor_unitario_ref : 0);

        if($requiereRef){
            if($vuRef<=0 && $precio>0){ $vuRef = $precio; }
            if($vuRef<0){ $vuRef = 0; }
        }else{
            $vuRef = 0;
        }
        $vtRef = round($vuRef * max($cti, 0), 2);
        $igvRef = ((float)$porc>0 && $requiereRef) ? round($vtRef * ((float)$porc/100), 2) : 0;

        $iddet = isset($det->iddetalle_venta) ? (int)$det->iddetalle_venta : 0;
        if($iddet>0){
            $updates = array(
                "cod_afectacion_igv='".$cod."'",
                "cod_tributo='".$codTrib."'",
                "porc_igv='".$porc."'"
            );
            if(_table_has_column('detalle_venta','valor_unitario_ref')){ $updates[] = "valor_unitario_ref='".number_format($vuRef,4,'.','')."'"; }
            if(_table_has_column('detalle_venta','valor_total_ref')){ $updates[] = "valor_total_ref='".number_format($vtRef,2,'.','')."'"; }
            if(_table_has_column('detalle_venta','igv_ref')){ $updates[] = "igv_ref='".number_format($igvRef,2,'.','')."'"; }
            if(_table_has_column('detalle_venta','es_gratuito')){ $updates[] = "es_gratuito='".(isset($conf['es_gratuito']) ? (int)$conf['es_gratuito'] : ($requiereRef?1:0))."'"; }
            if(_table_has_column('detalle_venta','es_bonificacion')){ $updates[] = "es_bonificacion='".(isset($conf['es_bonificacion']) ? (int)$conf['es_bonificacion'] : 0)."'"; }
            if(_table_has_column('detalle_venta','es_publicidad')){ $updates[] = "es_publicidad='".(isset($conf['es_publicidad']) ? (int)$conf['es_publicidad'] : 0)."'"; }
            if(_table_has_column('detalle_venta','es_retiro')){ $updates[] = "es_retiro='".(isset($conf['es_retiro']) ? (int)$conf['es_retiro'] : 0)."'"; }
            if(_table_has_column('detalle_venta','idcatalogo_afectacion')){ $updates[] = "idcatalogo_afectacion='".(isset($conf['id'])?(int)$conf['id']:0)."'"; }
            if(_table_has_column('detalle_venta','version_ubl')){ $updates[] = "version_ubl='".(isset($conf['version_ubl'])?trim((string)$conf['version_ubl']):'2.1')."'"; }
            if(_table_has_column('detalle_venta','codigo_tipo_precio')){ $updates[] = "codigo_tipo_precio='".(isset($conf['codigo_tipo_precio'])?trim((string)$conf['codigo_tipo_precio']):($requiereRef?'02':'01'))."'"; }
            if(_table_has_column('detalle_venta','codigo_leyenda')){ $updates[] = "codigo_leyenda='".(isset($conf['codigo_leyenda'])?trim((string)$conf['codigo_leyenda']):($requiereRef?'1002':''))."'"; }
            if(_table_has_column('detalle_venta','afecta_total_1004')){ $updates[] = "afecta_total_1004='".(isset($conf['requiere_total_1004'])?(int)$conf['requiere_total_1004']:($requiereRef?1:0))."'"; }
            if(_table_has_column('detalle_venta','base_imponible_xml')){ $updates[] = "base_imponible_xml='0.00'"; }
            if(_table_has_column('detalle_venta','monto_tributo_xml')){ $updates[] = "monto_tributo_xml='0.00'"; }
            if(_table_has_column('detalle_venta','valor_unitario_xml')){ $updates[] = "valor_unitario_xml='0.0000000000'"; }
            if(_table_has_column('detalle_venta','precio_unitario_xml')){ $updates[] = "precio_unitario_xml='0.0000000000'"; }
            if(_table_has_column('detalle_venta','valor_venta_xml')){ $updates[] = "valor_venta_xml='0.00'"; }

            $sqlu = "UPDATE detalle_venta SET ".implode(',', $updates)." WHERE iddetalle_venta='".$iddet."' LIMIT 1";
...
"2"=>$estadob,
"3"=>$reg->idventa,
"4"=>date("Y-m-d H:i:s", strtotime($reg->txtFECHA_DOCUMENTO)),
"5"=>$tdocumento,	
"6"=>$reg->txtSERIE.'-'.$reg->txtNUMERO,
"7"=>$nombre,
"8"=>$clientedoc,			
"9"=>$reg->txtSUB_TOTAL,
"10"=>$reg->txtIGV,
...
if($local['exonerado']>0){ $exoneradoigv=$local['exonerado']; }
...
//fin de buscador
        ...
        $records = ejecutarConsultaSimpleFila("select count(*) as allcount from articulo a INNER JOIN categoria c ON a.idcategoria=c.idcategoria  WHERE a.idempresa='$_COOKIE[id]' $idlocal2  AND a.condicion='1' AND a.medida!='ZZ'" . $searchQuery);
        $totalRecords = $records['allcount'];
...
## Fetch records
        $empQuery = "select a.txtCOD_ARTICULO, a.txtDESCRIPCION_ARTICULO, a.preciooferta, a.canjepuntos, a.medida, a.codigo, a.condicion, a.resaltado, a.exonerado_igv, a.mayor, a.precio, a.precio_mayor, a.precio_compra, a.comisionm, a.comisionmp, a.comision, a.existencia, a.moneda, a.idproveedor, a.canje, a.idcategoria from articulo  a INNER JOIN categoria c ON a.idcategoria=c.idcategoria  WHERE a.idempresa='$_COOKIE[id]' $idlocal2  AND a.condicion='1' AND a.medida != 'ZZ' " . $searchQuery . " ORDER BY a.txtCOD_ARTICULO ASC  limit " . $row . "," . $rowperpage;
        $empRecords = ejecutarConsulta($empQuery);
        //fin de buscador corelativo
    }
...
$imei.='<option value="'.$reg->medida.'|0|0|0|'.$preciofinal.'|'.$reg->mayor.'|'.$reg->comision.'|'.$reg->comisionm.'|'.$reg->comisionmp.'|'.$reg->txtCOD_ARTICULO.'|0">'.$preciofinal.' | UNIDAD</option>';
	
$datos = "SELECT *FROM articulo_unidad  WHERE idproducto='$reg->txtCOD_ARTICULO' ";
$datos = ejecutarConsulta($datos);
while($dato=mysqli_fetch_array($datos)) {
$imei.='<option value="'.$dato['medida'].'|'.$dato['cti'].'|'.$preciofinal.'|1|'.$preciofinal.'|'.$dato['ctimayor'].'|'.$dato['comision'].'|'.$dato['ctimayor'].'|'.$dato['comisionm'].'|'.$reg->txtCOD_ARTICULO.$dato['id'].'|'.$dato['id'].'">'.$preciofinal.' | '.$dato['nombre'].'</option>';
}
$imei.='</select>';
...
if($fa['precio_porcentaje']=='SI'){ $estilo='readonly'; }

if($exoneradoigv=='0'){

$selectedAfectacion='10';
if($reg->canje=='SI' && $cli['puntos']>=$reg->canjepuntos){
    $selectedAfectacion='35';
}elseif(trim((string)$reg->cod_afectacion_igv)!=''){
...
$sqlcat="SELECT * FROM categoria WHERE idcategoria='$reg->idcategoria' ";
$cate=ejecutarConsultaSimpleFila($sqlcat);	
...
"2"=>$tit,
"3"=>$lote,
"4"=>'<input id="p'.$reg->txtCOD_ARTICULO.'" class="form-control"  style="padding: 1px; margin: 1px;  height: 22px; " name="p'.$reg->txtCOD_ARTICULO.'" value="'.$preciofinal.'" type="text" '.$estilo.' > <input id="po'.$reg->txtCOD_ARTICULO.'" name="po'.$reg->txtCOD_ARTICULO.'" value="'.$preciofinal.'" type="hidden">',	
"5"=>$preciosinigv,	
"6"=>$preciocomprna,
"7"=>"<input type='text' class='form-control'  style='width: 40px;  padding: 1px; margin: 1px;  height: 22px; ' value='1' name='ctidet".$reg->txtCOD_ARTICULO."' id='ctidet".$reg->txtCOD_ARTICULO."' />",
"8"=>$botonstock,
"9"=>$tipoigv,	
"10"=>$codigoproveedor,
"11"=>'<button class="btn btn-danger btn-xs" onclick="agregartabla(\''.$reg->txtCOD_ARTICULO.'\', \''.$preciofinal.'\', \''.$texto.'\', \''.$reg->codigo.'\', \''.$stockf.'\', \'0\', \''.$reg->exonerado_igv.'\', \''.$reg->moneda.'\', \''.$reg->preciooferta.'\', \''.$reg->canjepuntos.'\')"><span class="fa fa-shopping-cart"></span></button>',
"12"=>$reg->codigo,
"13"=>$puntosf,
"14"=>$reg->canjepuntos,
"15"=>$reg->canje
...
normalizarDetalleAfectacionVenta($idventa);
...
$rspta = $resumen->detfactura($idventa);
	*/
        ...
        while ($reg = $rspta->fetch_object()) {
            ...
            $codAfectacionReplica = caf_extraer_codigo(isset($reg->cod_afectacion_igv) ? $reg->cod_afectacion_igv : '', caf_tipo_to_codigo(isset($reg->tipo) ? $reg->tipo : '10'));
            $confAfectacionReplica = caf_get_codigo_conf($codAfectacionReplica, isset($_COOKIE['id']) ? (int)$_COOKIE['id'] : 0);
            $codTributoReplica = isset($reg->cod_tributo) && trim((string)$reg->cod_tributo) !== '' ? trim((string)$reg->cod_tributo) : (string)$confAfectacionReplica['cod_tributo'];
            $porcIgvReplica = isset($reg->porc_igv) && trim((string)$reg->porc_igv) !== '' ? number_format((float)$reg->porc_igv, 2, '.', '') : number_format((float)$confAfectacionReplica['porcentaje_igv'], 2, '.', '');

            $colsDet = array('iddetalle_venta','idempresa','idlocal','beta','idventa','tipoarticulo','idproducto','codigoproducto','unidadmedida','idpresentacion','nombreproducto','txtCANTIDAD_ARTICULO','cantidadp','precio','preciocompra','descuento','subtotal','igv','ICB','importe','exoneradod','inafectad','gratuitad','detracciond','comisiond','stock','tipo','anticipo','doc_anticipo','placa','idlote','idproveedor','iddestino','carga_util','cantidad_toneladas','fecha','idcaja');
            $valsDet = array("NULL", "'$_COOKIE[id]'", "'$_COOKIE[idlocal]'", "'$reg->beta'", "'$idventanew'", "'$reg->tipoarticulo'", "'$reg->idproducto'", "'$reg->codigoproducto'", "'$reg->unidadmedida'", "'$reg->idpresentacion'", "'$reg->nombreproducto'", "'$reg->txtCANTIDAD_ARTICULO'", "'$reg->cantidadp'", "'$reg->precio'", "'$reg->preciocompra'", "'".(isset($reg->descuento)?$reg->descuento:(isset($reg->dto_item_monto)?$reg->dto_item_monto:'0.00'))."'", "'$reg->subtotal'", "'$reg->igv'", "'$reg->ICB'", "'$reg->importe'", "'$reg->exoneradod'", "'$reg->inafectad'", "'$reg->gratuitad'", "'$reg->detracciond'", "'$reg->comisiond'", "'$reg->stock'", "'$reg->tipo'", "'$reg->anticipo'", "'$reg->doc_anticipo'", "'$reg->placa'", "'$reg->idlote'", "'$reg->idproveedor'", "'$reg->iddestino'", "'$reg->carga_util'", "'$reg->cantidad_toneladas'", "'$reg->fecha'", "'$reg->idcaja'");

            $optRep = array(
                'cod_afectacion_igv'=>"'$codAfectacionReplica'",
                'cod_tributo'=>"'$codTributoReplica'",
                'porc_igv'=>"'$porcIgvReplica'",
                'valor_unitario_ref'=>"'".(isset($reg->valor_unitario_ref)?$reg->valor_unitario_ref:'0')."'",
                'igv_ref'=>"'".(isset($reg->igv_ref)?$reg->igv_ref:'0')."'",
                'valor_total_ref'=>"'".(isset($reg->valor_total_ref)?$reg->valor_total_ref:'0')."'",
                'es_retiro'=>"'".(isset($reg->es_retiro)?$reg->es_retiro:'0')."'",
                'es_publicidad'=>"'".(isset($reg->es_publicidad)?$reg->es_publicidad:'0')."'",
                'es_bonificacion'=>"'".(isset($reg->es_bonificacion)?$reg->es_bonificacion:'0')."'",
                'es_gratuito'=>"'".(isset($reg->es_gratuito)?$reg->es_gratuito:'0')."'",
                'idcatalogo_afectacion'=>"'".(isset($reg->idcatalogo_afectacion)?$reg->idcatalogo_afectacion:(isset($confAfectacionReplica['id'])?(int)$confAfectacionReplica['id']:0))."'",
                'version_ubl'=>"'".(isset($reg->version_ubl)?$reg->version_ubl:(isset($confAfectacionReplica['version_ubl'])?$confAfectacionReplica['version_ubl']:'2.1'))."'",
                'codigo_tipo_precio'=>"'".(isset($reg->codigo_tipo_precio)?$reg->codigo_tipo_precio:(isset($confAfectacionReplica['codigo_tipo_precio'])?$confAfectacionReplica['codigo_tipo_precio']:'01'))."'",
                'base_imponible_xml'=>"'".(isset($reg->base_imponible_xml)?$reg->base_imponible_xml:'0.00')."'",
                'monto_tributo_xml'=>"'".(isset($reg->monto_tributo_xml)?$reg->monto_tributo_xml:'0.00')."'",
                'valor_unitario_xml'=>"'".(isset($reg->valor_unitario_xml)?$reg->valor_unitario_xml:'0.0000000000')."'",
                'precio_unitario_xml'=>"'".(isset($reg->precio_unitario_xml)?$reg->precio_unitario_xml:'0.0000000000')."'",
                'valor_venta_xml'=>"'".(isset($reg->valor_venta_xml)?$reg->valor_venta_xml:'0.00')."'",
                'codigo_leyenda'=>"'".(isset($reg->codigo_leyenda)?$reg->codigo_leyenda:(isset($confAfectacionReplica['codigo_leyenda'])?$confAfectacionReplica['codigo_leyenda']:''))."'",
                'afecta_total_1004'=>"'".(isset($reg->afecta_total_1004)?$reg->afecta_total_1004:(isset($confAfectacionReplica['requiere_total_1004'])?(int)$confAfectacionReplica['requiere_total_1004']:0))."'",
                'dto_item_monto'=>"'".(isset($reg->dto_item_monto)?$reg->dto_item_monto:(isset($reg->descuento)?$reg->descuento:'0.00'))."'",
                'dto_item_tipo'=>"'".(isset($reg->dto_item_tipo)?$reg->dto_item_tipo:'MONTO')."'",
                'dto_item_valor'=>"'".(isset($reg->dto_item_valor)?$reg->dto_item_valor:'0.0000')."'",
                'dto_item_monto_base'=>"'".(isset($reg->dto_item_monto_base)?$reg->dto_item_monto_base:'0.00')."'",
                'dto_item_monto_igv'=>"'".(isset($reg->dto_item_monto_igv)?$reg->dto_item_monto_igv:'0.00')."'"
            );
            ...
            foreach($optRep as $c=>$v){ if(in_array($c,$criticalCols,true) || _table_has_column('detalle_venta',$c)){ $colsDet[]=$c; $valsDet[]=$v; } }

            $sql_detalle = "INSERT INTO detalle_venta (".implode(',', $colsDet).") VALUES (".implode(',', $valsDet).")";
