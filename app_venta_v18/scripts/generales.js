...
function _updateCamposDesdeSaldo(resp){
	...
	if(resp && resp.medio_pago){
		$('#tpago').val(resp.medio_pago);
	}else if(resp && resp.tipo_pago){
	...
	try{
		if(_num(resp && resp.detraccion) > 0){ $('#tipopago').val('1'); }
		else if(_num(resp && resp.retencion) > 0){ $('#tipopago').val('2'); }
		else{ $('#tipopago').val('0'); }
	}catch(e){}
}
