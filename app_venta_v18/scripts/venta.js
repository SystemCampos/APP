...
porigv=porigv+1.00;	
var impuesto=porigv;
var catalogoAfectacionesVentaMap = {};
if(typeof window.CATALOGO_AFECTACIONES_VENTA !== 'undefined' && window.CATALOGO_AFECTACIONES_VENTA){
	for(var _iCat=0; _iCat<window.CATALOGO_AFECTACIONES_VENTA.length; _iCat++){
		var _itCat = window.CATALOGO_AFECTACIONES_VENTA[_iCat] || {};
		if(_itCat.codigo){
			catalogoAfectacionesVentaMap[String(_itCat.codigo)] = {
				cod_tributo: String(_itCat.cod_tributo || ''),
				porc_igv: parseFloat(_itCat.porcentaje_igv || 0).toFixed(2),
				descripcion: String(_itCat.descripcion || '')
			};
		}
	}
}
...
function _setSelect2OptionSilentVenta(selector, value, text){
  var $el = $(selector);
  if(!$el.length){ return; }
  ...
  var txt = String(text || val).trim();
  var exists = false;
  $el.find('option').each(function(){
    if(String($(this).val()) === val){
      $(this).prop('selected', true).text(txt);
      exists = true;
      return false;
    }
  });
  ...
  try{ $el.trigger('change.select2'); }catch(e){ try{ $el.trigger('change'); }catch(e2){} }
}

function _aplicarFpagoEditVenta(idFpago, txtFpago, idMedioFallback, txtMedioFallback, pagoformaFallback){
  var idtpp = String(idFpago || '').trim();
  if(!idtpp){
    if(idMedioFallback){ _setSelect2OptionSilentVenta('#medio', idMedioFallback, txtMedioFallback || idMedioFallback); }
    if(pagoformaFallback){ $('#pago').val(pagoformaFallback).trigger('change'); }
    return;
  }

  _setSelect2OptionSilentVenta('#fpago_mpago', idtpp, txtFpago || idtpp);
    ...
    .done(function(resp){
      if(resp && resp.ok === true){
        if(typeof resp.id_pago !== 'undefined' && resp.id_pago !== null && String(resp.id_pago) !== ''){
          _setSelect2OptionSilentVenta('#medio', resp.id_pago, resp.forma_descripcion || resp.id_pago);
        }else if(idMedioFallback){
          _setSelect2OptionSilentVenta('#medio', idMedioFallback, txtMedioFallback || idMedioFallback);
        }
        if(resp.pagoforma){
          $('#pago').val(String(resp.pagoforma).toUpperCase()).trigger('change');
        }else if(pagoformaFallback){
          $('#pago').val(pagoformaFallback).trigger('change');
        }
      }else{
        if(idMedioFallback){ _setSelect2OptionSilentVenta('#medio', idMedioFallback, txtMedioFallback || idMedioFallback); }
        if(pagoformaFallback){ $('#pago').val(pagoformaFallback).trigger('change'); }
...
function _validarClienteSeleccion(){
  var v = String($('#txtID_CLIENTE').val() || '').trim();
  if(v === '' || v === '0' || v.toLowerCase() === 'null'){
    var msg = 'Debe seleccionar Cliente Comprobante antes de agregar productos o servicios.';
    try{
      if(typeof Swal !== 'undefined' && Swal.fire){ Swal.fire({icon:'warning', title:'Validaci�n', text: msg}); }
      else{ alert(msg); }
    }catch(e){ alert(msg); }
    return false;
  }
...
function _generarCuotasSegunFpago(done){
  ...
  $.getJSON('data/venta.php', { op: 'cfg_tipopago_persona', id: fpago })
    .done(function(cfg){
      ...
      var total = _obtenerTotalCuotasVenta();
...
// ======================================================
// SUNAT - Redondeo de detracción a la UNIDAD
// Regla:
// - Si la fracción < 0.5 => se suprime decimal (queda igual)
// - Si la fracción >= 0.5 => se ajusta a la unidad inmediata superior
// Nota: Se aplica sobre valores positivos (detracción/retención son >=0)
// ======================================================
function _roundSunatUnidad(valor){
  var n = parseFloat(valor);
  if(isNaN(n) || !isFinite(n)) return 0;
  if(n <= 0) return 0;
  var ent = Math.floor(n);
  var frac = n - ent;
...
function _comprimirSidebarVenta(){
    try{
        var $body = $('body.page-body');
        if(!$body.hasClass('sidebar-collapsed')){
            var $toggle = $('.entypo-menu:visible').first();
...
function nuevo() {
    _comprimirSidebarVenta();
    try{
        _nuevaVentaTipo = String($('#txtID_TIPO_DOCUMENTO').val() || '03');
        _nuevaVentaSerie = String($('#txtSERIE').val() || '');
        _nuevaVentaPreservaComprobante = true;
        $('#nuevaventa2').prop('disabled', true);
        $('.guardaventa').prop('disabled', false);
        $('#agregar2,#agregars2,#guardar2,#guardarenv2').prop('disabled', false);
        $('#txtID_TIPO_DOCUMENTO').prop('disabled', false);
...
var _nuevaVentaPreservaComprobante = false;
var _nuevaVentaTipo = "03";
var _nuevaVentaSerie = "";
...
function _getTasaIgvGlobalVenta(){
	var tasa = 0.18;
	try{
		$("#detpedidos input[id^='idp']").each(function(){
			var idp = String(this.id || '').replace('idp','');
			if(!idp){ return; }
			var cod = String($('#tipo'+idp).val() || '').trim();
			if(cod === '10'){
				var p = _moneyNum($('#porcigv'+idp).val());
				if(p > 0){ tasa = p / 100; }
				return false;
			}
		});
	}catch(e){}
	if(!isFinite(tasa) || tasa < 0){ tasa = 0.18; }
	return tasa;
}

function _descomponerDescuentoGlobalSunat(descuentoTotal, tasaIgv){
	descuentoTotal = _moneyNum(descuentoTotal);
	tasaIgv = _moneyNum(tasaIgv);
	if(!isFinite(tasaIgv) || tasaIgv < 0){ tasaIgv = 0.18; }
	if(descuentoTotal <= 0){
		return { base: 0, igv: 0, total: 0, factor: 0 };
	}
	var factor = 1 + tasaIgv;
	if(!isFinite(factor) || factor <= 0){ factor = 1.18; }
	var base = Math.round((descuentoTotal / factor) * 100) / 100;
	var igv  = Math.round((descuentoTotal - base) * 100) / 100;
	return {
		base: base,
		igv: igv,
...
function _obtenerTotalCuotasVenta(){
	var totalDoc = _moneyNum($('#txtTOTAL').val());
	var detraccion = _moneyNum($('#montodetraccion').val());
	var retencion = _moneyNum($('#retencion').val());
	var totalCuotas = totalDoc - detraccion - retencion;
	if(totalCuotas < 0){ totalCuotas = 0; }
	return parseFloat(totalCuotas.toFixed(2));
}

// ======================================================
// Recalcular montos de cuotas cuando cambian Detracción o Retención
// Regla: Base cuotas = TOTAL - Detracción - Retención
// - Si ya hay cuotas generadas (tabla DataTable "t"), ajusta SOLO montos.
// - Conserva: tipo pago, fechas, moneda, tcambio.
// ======================================================
function _recalcularCuotasPorDetRet(){
	try{
		var pagoSel = String($('#pago').val() || '').toUpperCase();
		if(pagoSel.indexOf('CRED') === -1){ return; }
		if(typeof t === 'undefined' || !t || !t.rows){ return; }
		var n = t.rows().count();
		if(!n || n <= 0){ return; }
		...
		for(var i=0; i<cuotas; i++){
			var monto = base;
			if(i === (cuotas-1)){
				monto = Math.round((total - acum) * 100) / 100;
			}
			acum = Math.round((acum + monto) * 100) / 100;
			// Col 4 = monto (según add en _generarCuotasSegunFpago)
			data[i][4] = _moneyFix2(monto);
		}
...
function _sincronizarDescuentoGlobalVenta(valorEditable){
	var bruto = (typeof valorEditable !== 'undefined') ? _moneyNum(valorEditable) : _moneyNum($('#descuentotot').val());
	if(bruto < 0){ bruto = 0; }
	bruto = Math.round(bruto * 100) / 100;
	$('#dto_global_tipo').val('MONTO');
	$('#dto_global_valor').val(bruto.toFixed(4));
	$('#dto_global_aplica_antes_igv').val('1');
	return bruto;
}

function _tipoCalculoLegacyVenta(tipo){
	tipo = String(tipo || '').trim();
	if(tipo==='10' || tipo===''){ return '0'; }
	if(tipo==='20'){ return '1'; }
	if(tipo==='30'){ return '3'; }
	if(['11','12','13','14','15','16','21','31','32','33','34','35','36'].indexOf(tipo)>=0){ return '2'; }
	return tipo;
}

function _tipoToAfectacionVenta(tipo){
	tipo = String(tipo || '').trim();
	if(['10','11','12','13','14','15','16','20','21','30','31','32','33','34','35','36'].indexOf(tipo)>=0){ return tipo; }
	if(tipo==='1'){ return '20'; }
	if(tipo==='2'){ return '35'; }
	if(tipo==='3'){ return '30'; }
	return '10';
...
function _getPorcIgvDocumentoVenta(){
	var p = _moneyNum($('#porcentajeigv').val());
	if(!isFinite(p) || p <= 0){ p = 18; }
	return p.toFixed(2);
}

function _tributoPorAfectacionVenta(cod){
	cod = String(cod || '').trim();
	if(catalogoAfectacionesVentaMap[cod]){
		var it=catalogoAfectacionesVentaMap[cod]||{};
		return {
			cod_tributo:String(it.cod_tributo || '1000'),
			porc_igv:parseFloat(it.porc_igv || 0).toFixed(2),
			requiere_valor_referencial:parseInt(it.requiere_valor_referencial||0,10)?1:0,
			codigo_tipo_precio:String(it.codigo_tipo_precio || ((parseInt(it.es_onerosa||1,10)===0)?'02':'01')),
			es_gratuito:parseInt(it.es_gratuito||0,10)?1:0,
			idcatalogo_afectacion:String(it.id||0)
		};
	}
	if(cod==='10'){ return {cod_tributo:'1000', porc_igv:_getPorcIgvDocumentoVenta(), requiere_valor_referencial:0, codigo_tipo_precio:'01', es_gratuito:0, idcatalogo_afectacion:'0'}; }
	if(['11','12','13','14','15','16','21','31','32','33','34','35','36'].indexOf(cod)>=0){ return {cod_tributo:'9996', porc_igv:_getPorcIgvDocumentoVenta(), requiere_valor_referencial:1, codigo_tipo_precio:'02', es_gratuito:1, idcatalogo_afectacion:'0'}; }
	if(cod==='20'){ return {cod_tributo:'9997', porc_igv:'0.00', requiere_valor_referencial:0, codigo_tipo_precio:'01', es_gratuito:0, idcatalogo_afectacion:'0'}; }
	if(cod==='30'){ return {cod_tributo:'9998', porc_igv:'0.00', requiere_valor_referencial:0, codigo_tipo_precio:'01', es_gratuito:0, idcatalogo_afectacion:'0'}; }
	if(cod==='40'){ return {cod_tributo:'9995', porc_igv:'0.00', requiere_valor_referencial:0, codigo_tipo_precio:'01', es_gratuito:0, idcatalogo_afectacion:'0'}; }
	return {cod_tributo:'1000', porc_igv:_getPorcIgvDocumentoVenta(), requiere_valor_referencial:0, codigo_tipo_precio:'01', es_gratuito:0, idcatalogo_afectacion:'0'};
}

function _esAfectacionNoOnerosaVenta(cod){
	cod = String(cod || '').trim();
	return (['11','12','13','14','15','16','21','31','32','33','34','35','36'].indexOf(cod)>=0);
}

function _normalizarDescripcionArticuloVenta(txt){
  var base = String(txt || '');
  return base.replace(/\s*\((Bonif\.|Ret\.xPub\.|Gratuito)\)\s*$/i, '').trim();
}

function _etiquetaAfectacionVenta(cod){
  cod = String(cod || '').trim();
  if(cod === '15' || cod === '31'){ return ' (Bonif.)'; }
  if(cod === '14' || cod === '36'){ return ' (Ret.xPub.)'; }
  if(['11','12','13','16','21','32','33','34','35','2'].indexOf(cod) >= 0){ return ' (Gratuito)'; }
  return '';
}

function _descripcionConAfectacionVenta(txt, cod){
  var base = _normalizarDescripcionArticuloVenta(txt);
  var suf = _etiquetaAfectacionVenta(_tipoToAfectacionVenta(cod));
  return base + suf;
}

function _asegurarHiddenDetalleVenta(id, name, value){
	var $el = $('#'+name+id);
	if(!$el.length){
		var $td = $('#idp'+id).closest('td');
		if($td.length){
			$td.append(' <input type="hidden" id="'+name+id+'" name="'+name+id+'" value="'+value+'">');
			$el = $('#'+name+id);
		}
	}
...
function _sincronizarSunatDetalleVenta(id, codAfectacion, precioUnitario, cantidad){
	codAfectacion = _tipoToAfectacionVenta(codAfectacion);
	var trib = _tributoPorAfectacionVenta(codAfectacion);
	var precioNum = _moneyNum(precioUnitario);
	var cantidadNum = _moneyNum(cantidad);
	var valorUnitRef = '0.000000';
	var valorTotalRef = '0.00';
	var igvRef = '0.00';
	if(_esAfectacionNoOnerosaVenta(codAfectacion)){
		valorUnitRef = precioNum.toFixed(6);
		valorTotalRef = _moneyFix2(precioNum * cantidadNum);
		if(codAfectacion==='14' || codAfectacion==='15'){
			igvRef = _moneyFix2(_moneyNum(valorTotalRef) * (_moneyNum(trib.porc_igv) / 100));
		}
		if($('#sub'+id).length){ $('#sub'+id).val('0.00'); }
		if($('#igv'+id).length){ $('#igv'+id).val('0.00'); }
		if($('#gravadas'+id).length){ $('#gravadas'+id).val('0.00'); }
		if($('#totf'+id).length){ $('#totf'+id).val('0.00'); }
		if($('#grat'+id).length){ $('#grat'+id).val(valorTotalRef); }
	}
	$('#tipo'+id).val(codAfectacion);
	_asegurarHiddenDetalleVenta(id, 'codafectacion', codAfectacion);
	_asegurarHiddenDetalleVenta(id, 'codtributo', trib.cod_tributo);
	_asegurarHiddenDetalleVenta(id, 'porcigv', trib.porc_igv);
	_asegurarHiddenDetalleVenta(id, 'valorunitref', valorUnitRef);
	_asegurarHiddenDetalleVenta(id, 'igvref', igvRef);
	_asegurarHiddenDetalleVenta(id, 'valortotalref', valorTotalRef);
	if($('#precioo'+id).length){ $('#precioo'+id).val(precioNum.toFixed(6)); }
}


function actualizarDescuentoGlobalVenta(){
	_sincronizarDescuentoGlobalVenta($('#descuentotot').val());
	totales();
}
...
$(document).ready(function(){
	...
	$(document).on('focus', '#descuentotot', function(){
		$(this).val(_moneyRaw($(this).val()));
		this.select();
	});
	$(document).on('blur', '#descuentotot', function(){
		actualizarDescuentoGlobalVenta();
	});
	$(document).on('change', '#descuentotot', function(){
		actualizarDescuentoGlobalVenta();
	});
});
...
//Función limpiar
function limpiar(){
	var _tipoPrev = (_nuevaVentaPreservaComprobante ? String(_nuevaVentaTipo||'03') : '03');
	var _seriePrev = (_nuevaVentaPreservaComprobante ? String(_nuevaVentaSerie||'') : '');
	$("#txtID_TIPO_DOCUMENTO").val(_tipoPrev);
	$("#txtID_CLIENTE").val("");
	$("#cliente").val("");
	if(!_nuevaVentaPreservaComprobante){
		$("#txtID_TIPO_DOCUMENTO").val("");
		$("#txtSERIE").val("");
	}else{
		$("#txtID_TIPO_DOCUMENTO").val(_tipoPrev);
		$("#txtSERIE").val(_seriePrev);
	}
	$("#txtNUMERO").val("")
	$("#txtTIPO_DOCUMENTO_CLIENTE").val("");
	$("#txtOBSERVACION").val("");
	$("#txtSUB_TOTAL").val("0.00");
	$("#txtIGV").val("");
	$("#txtTOTAL").val("0.00");
	$("#descuentotot").val("0.00");
	$("#dto_global_tipo").val("MONTO");
	$("#dto_global_valor").val("0.0000");
	$("#dto_global_monto_base").val("0.00");
	$("#dto_global_monto_igv").val("0.00");
	$("#dto_global_aplica_antes_igv").val("1");
	$("#txtID_MONEDA").val("");
	...
	$("#txtID_MOTIVO").val("");
	$(".filas").remove();
	try{ if(typeof tabladetalles !== 'undefined' && tabladetalles && tabladetalles.clear){ tabladetalles.clear().draw(false); } }catch(e){}
	try{ if(typeof tablasaldo !== 'undefined' && tablasaldo && tablasaldo.clear){ tablasaldo.clear().draw(false); } }catch(e){}
	$("#total").html("0");
	$("#idventa").val("0");
$("#valref").val("0");
$("#tipopaso").val("0");
...
try{
	$('#fpago_mpago').val(null).trigger('change');
	$('#medio').val(null).trigger('change');
	$('#pago').val('').trigger('change');
	$('#txtMONEDA').val('PEN').trigger('change');
}catch(e){}
...
//Obtenemos la fecha actual
    ...
    if(_nuevaVentaPreservaComprobante){ _nuevaVentaPreservaComprobante = false; }
...
if (flag){
		_comprimirSidebarVenta();
		if($menuToggle.length && !$('body.page-body').hasClass('sidebar-collapsed')){
...
var _tipoIni = String($('#txtID_TIPO_DOCUMENTO').val()||(_nuevaVentaPreservaComprobante?_nuevaVentaTipo:'03'));
if(!_tipoIni){ _tipoIni='03'; }
$('#txtID_TIPO_DOCUMENTO').val(_tipoIni);
seriesfinales(_tipoIni);
serie(1);	
}else{
...
if(data.tipo_pago){ $('#pago').val(data.tipo_pago).trigger('change'); }
if(data.medio_pago){ _setSelect2OptionSilentVenta('#medio', data.medio_pago, data.medio_pago_text || data.medio_pago); }
...
$("#txtSUB_TOTAL").val((typeof data.txtSUB_TOTAL!=="undefined" && data.txtSUB_TOTAL!==null) ? data.txtSUB_TOTAL : "0.00");
$("#txtIGV").val(data.txtIGV);
$("#txtTOTAL").val(data.txtTOTAL);
$("#exoneradof").val((typeof data.exonerado!=="undefined" && data.exonerado!==null) ? data.exonerado : "0.00");
$("#gratuita").val((typeof data.gratuita!=="undefined" && data.gratuita!==null) ? data.gratuita : "0.00");
$("#inafecta").val((typeof data.inafecta!=="undefined" && data.inafecta!==null) ? data.inafecta : "0.00");
var dtoGlobalMontoMostrar=((typeof data.dto_global_monto!=="undefined" && data.dto_global_monto!==null) ? data.dto_global_monto : ((typeof data.descuento!=="undefined" && data.descuento!==null) ? data.descuento : "0.00"));
$("#descuentotot").val(dtoGlobalMontoMostrar);
$("#dto_global_tipo").val((typeof data.dto_global_tipo!=="undefined" && data.dto_global_tipo!==null && String(data.dto_global_tipo).trim()!=="") ? data.dto_global_tipo : "MONTO");
$("#dto_global_valor").val((typeof data.dto_global_valor!=="undefined" && data.dto_global_valor!==null) ? data.dto_global_valor : _moneyNum(dtoGlobalMontoMostrar).toFixed(4));
$("#dto_global_monto_base").val((typeof data.dto_global_monto_base!=="undefined" && data.dto_global_monto_base!==null) ? data.dto_global_monto_base : "0.00");
$("#dto_global_monto_igv").val((typeof data.dto_global_monto_igv!=="undefined" && data.dto_global_monto_igv!==null) ? data.dto_global_monto_igv : "0.00");
$("#dto_global_aplica_antes_igv").val((typeof data.dto_global_aplica_antes_igv!=="undefined" && data.dto_global_aplica_antes_igv!==null) ? data.dto_global_aplica_antes_igv : "1");
...
var _fpagoIdEdit = (data.fpago_mpago && String(data.fpago_mpago).trim()!=='') ? data.fpago_mpago : data.guia5;
var _fpagoTxtEdit = (typeof data.fpago_mpago_text !== 'undefined' && data.fpago_mpago_text !== null && String(data.fpago_mpago_text).trim()!=='') ? data.fpago_mpago_text : _fpagoIdEdit;
_aplicarFpagoEditVenta(_fpagoIdEdit, _fpagoTxtEdit, data.medio_pago, data.medio_pago_text || data.medio_pago, data.tipo_pago);
	_aplicarFormatoTotalesVenta();
...
//var pre=precio;
	...
	tipo=_tipoCalculoLegacyVenta(tipo);
...
//$('#detpedidos').dataTable().fnUpdate(total, idrow, 4);
	if(tipo=='2'){
		var totalRef = total;
		if(descuento>'0.00'){
			totalRef = parseFloat(totalRef - descuento);
			if(totalRef < 0){ totalRef = 0; }
		}
		sub='0.00';
		igv='0.00';
		exon='0.00';
...
//$('#descuento'+id).val(descuentofinal);
	...
	_sincronizarSunatDetalleVenta(id, ($('#codafectacion'+id).length ? $('#codafectacion'+id).val() : $('#tipo'+id).val()), pre, cti);
...
function totales(exonerada){
	...
	var ctif=0; var totgratuita='0.00'; var descuentoItems='0.00'; var sumtotaldesc='0.00';
	var baseDetraccion='0.00';
	var igv2='0.00';
	var subtotal2='0.00';
	var total='0.00';
	var total='0.00';
		...
		$(this).children("td").each(function(index2){
			switch(index2){
				...
				case 0:
					...
					tipo=$('#tipo'+idu).val();
					tipo=_tipoCalculoLegacyVenta(tipo);
					descuentoItems=_moneyNum(descuentoItems)+_moneyNum($('#descuento'+idu).val());
console.log('tipo:'+tipo);
					if(tipo=='2'){

						precioo=$('#precioo'+idu).val();
						ctif=$('#ctif'+idu).val();
						totgratuita = ($('#valortotalref'+idu).length ? _moneyNum($('#valortotalref'+idu).val()) : (_moneyNum(precioo) * _moneyNum(ctif)));
						gratuita = _moneyNum(gratuita) + _moneyNum(totgratuita);
						valref = _moneyNum(valref) + _moneyNum(totgratuita);
						$('#sub'+idu).val('0.00');
						$('#igv'+idu).val('0.00');
						$('#gravadas'+idu).val('0.00');
						$('#totf'+idu).val('0.00');
						$('#exo'+idu).val('0.00');
						$('#inafecta'+idu).val('0.00');
						$('#grat'+idu).val(_moneyFix2(totgratuita));
					...
					baseDetraccion=_moneyNum(baseDetraccion)+_moneyNum($('#totdetraccion'+idu).val());
	...
	var gravadasRaw = _moneyNum(gravadas);
	var subtotalRaw = _moneyNum(subtotal);
	var igvRaw = _moneyNum(igv);
	var sumtotalRaw = _moneyNum(sumtotal);
	var descuentoGlobal = _sincronizarDescuentoGlobalVenta($('#descuentotot').val());
	var baseGravadaBruta = gravadasRaw;
	var igvBruto = igvRaw;
	var maxDescuentoGlobal = baseGravadaBruta + igvBruto;
	if(descuentoGlobal > maxDescuentoGlobal){
		descuentoGlobal = maxDescuentoGlobal;
	}
	if(descuentoGlobal < 0){ descuentoGlobal = 0; }

	var factorGlobal = 1;
	if(baseGravadaBruta > 0 && igvBruto >= 0){
		factorGlobal = (baseGravadaBruta + igvBruto) / baseGravadaBruta;
		if(!isFinite(factorGlobal) || factorGlobal <= 0){ factorGlobal = 1; }
	}
	var descuentoGlobalBase = 0;
	var descuentoGlobalIgv = 0;
	if(descuentoGlobal > 0){
		if(baseGravadaBruta > 0){
			descuentoGlobalBase = Math.round((descuentoGlobal / factorGlobal) * 100) / 100;
			descuentoGlobalIgv = Math.round((descuentoGlobal - descuentoGlobalBase) * 100) / 100;
		}else{
			descuentoGlobalBase = 0;
			descuentoGlobalIgv = 0;
			descuentoGlobal = 0;
		}
	}
	if(descuentoGlobalBase > baseGravadaBruta){ descuentoGlobalBase = baseGravadaBruta; }
	if(descuentoGlobalIgv > igvBruto){ descuentoGlobalIgv = igvBruto; }

	gravadasRaw = Math.max(0, gravadasRaw - descuentoGlobalBase);
	subtotalRaw = Math.max(0, subtotalRaw - descuentoGlobalBase);
	igvRaw = Math.max(0, igvRaw - descuentoGlobalIgv);
	sumtotalRaw = Math.max(0, sumtotalRaw - descuentoGlobal);

	$('#dto_global_monto_base').val(descuentoGlobalBase.toFixed(2));
	$('#dto_global_monto_igv').val(descuentoGlobalIgv.toFixed(2));
	$('#dto_global_valor').val(descuentoGlobal.toFixed(4));

	if(operacionretenciones=='1'){
		retencion=(parseFloat(3)*parseFloat(sumtotalRaw))/100;
	...
	if(detracciones!='0|0|0'){
		...
		if(_moneyNum(baseDetraccion)>sumtotalRaw){
			totdetraccion=parseFloat(porcentaje)*parseFloat(baseDetraccion)/100;
		}else{
			totdetraccion=parseFloat(porcentaje)*parseFloat(sumtotalRaw)/100;
		}

		var moneda=$('#txtMONEDA').val();

		if(moneda==='PEN'){
			totdetraccion = _roundSunatUnidad(totdetraccion);
			totdetraccion = String(totdetraccion);
		}else{
			totdetraccion = parseFloat(totdetraccion||0).toFixed(2);
		}
}

	var exoneradoRaw=_moneyNum(exonerado);
	var totinafectaRaw=_moneyNum(totinafecta);
	var comisionRaw=_moneyNum(comision);
	var gratuitaRaw=_moneyNum(gratuita);
	var ofertaRaw=_moneyNum(oferta);
	var puntosRaw=_moneyNum(puntos);
	var valrefRaw=_moneyNum(valref);

	if(percepcionsi=='1'){
		percepcion=(parseFloat(2)*parseFloat(sumtotalRaw))/100;
		percepcion=percepcion.toFixed(2);
	}

console.log('gravadastotales:'+gravadasRaw);

	var totalDocumento=_moneyNum(sumtotalRaw)+_moneyNum(percepcion);
	if(totalDocumento<0){ totalDocumento=0; }
	totalDocumento=totalDocumento.toFixed(2);
	var totalNetoPagar=_moneyNum(totalDocumento)-_moneyNum(retencion);
	if(totalNetoPagar<0){ totalNetoPagar=0; }
	totalNetoPagar=totalNetoPagar.toFixed(2);

	$('#descuentotot').val(descuentoGlobal.toFixed(2));
	$('#puntos').val(puntosRaw.toFixed(2));
	$('#txtSUB_TOTAL').val(subtotalRaw.toFixed(2));
	$('#gravadas').val(gravadasRaw.toFixed(2));
	$('#exoneradof').val(exoneradoRaw.toFixed(2));
	$('#gratuita').val(gratuitaRaw.toFixed(2));
	$('#txtIGV').val(igvRaw.toFixed(2));
	$('#txtTOTAL').val(totalDocumento);
	$('#totalf').val(totalDocumento);
	$('#totalpagarf').html(totalDocumento);
	$('#comisiont').val(comisionRaw.toFixed(2));
	$('#inafecta').val(totinafectaRaw.toFixed(2));
	$('#retencion').val(retencion);
	$('#montodetraccion').val(totdetraccion);
	$('#totpagar').val(_obtenerTotalCuotasVenta().toFixed(2));
	$('#percepcion').val(percepcion);
	$('#valref').val(valrefRaw.toFixed(2));
	_aplicarFormatoTotalesVenta();
	_recalcularCuotasPorDetRet();
...
codafectacion=(typeof result[i].cell[25] !== 'undefined') ? result[i].cell[25] : tipoope;
codtributo=(typeof result[i].cell[26] !== 'undefined') ? result[i].cell[26] : '';
porcigv=(typeof result[i].cell[27] !== 'undefined') ? result[i].cell[27] : '';
valorunitarioref=(typeof result[i].cell[28] !== 'undefined') ? result[i].cell[28] : '0.000000';
igvref=(typeof result[i].cell[29] !== 'undefined') ? result[i].cell[29] : '0.00';
valortotalref=(typeof result[i].cell[30] !== 'undefined') ? result[i].cell[30] : '0.00';
articulo = _descripcionConAfectacionVenta(articulo, codafectacion);

var campo='<input type="hidden" id="idp'+idu+'" name="idp'+idu+'" value="'+id+'"> <input type="hidden" id="tipoart'+idu+'" name="tipoart'+idu+'" value="'+tipoarticulo+'"> <input type="hidden" id="codigo'+idu+'" name="codigo'+idu+'" value="'+codigo+'"> <input type="hidden" id="umedida'+idu+'" name="umedida'+idu+'" value="'+umedida+'"> <input type="hidden" id="tit'+idu+'" name="tit'+idu+'" value="'+articulo+'"> <input type="hidden" id="cti2'+idu+'" name="cti2'+idu+'" value="'+cti2+'"> <input type="hidden" id="exon'+idu+'" name="exon'+idu+'" value="'+exonerado+'"> <input type="hidden" id="tipo'+idu+'" name="tipo'+idu+'" value="'+codafectacion+'"> <input type="hidden" id="igvh'+idu+'" name="igvh'+idu+'" value="'+igv+'"> <input type="hidden" id="exo'+idu+'" name="exo'+idu+'" value="'+exonf+'">  <input type="hidden" id="gravadas'+idu+'" name="gravadas'+idu+'" value="'+gravadas+'"> <input type="hidden" id="comision'+idu+'" name="comision'+idu+'" value="'+comision+'"> <input type="hidden" id="idu'+id+'" name="idu'+id+'" value="'+idu+'"> <input type="hidden" id="idunid'+idu+'" name="idunit'+idu+'" value="'+idunit+'"> <input type="hidden" id="grat'+idu+'" name="grat'+idu+'" value="'+gratuitas+'"> <input type="hidden" id="descuento'+idu+'" name="descuento'+idu+'" value="0.00"> <input type="hidden" id="puntos'+idu+'" name="puntos'+idu+'" value="0"> <input type="hidden" id="ctipuntos'+idu+'" name="ctipuntos'+idu+'" value="0">  <input type="hidden" id="totdetraccion'+idu+'" name="totdetraccion'+idu+'" value="'+detracciond+'">   <input type="hidden" id="cantidad_toneladas'+idu+'" name="cantidad_toneladas'+idu+'" value="'+cantidad_toneladas+'">  <input type="hidden" id="carga_util'+idu+'" name="carga_util'+idu+'" value="'+carga_util+'">  <input type="hidden" id="iddestino'+idu+'" name="iddestino'+idu+'" value="'+iddestino+'"> <input type="hidden" id="codafectacion'+idu+'" name="codafectacion'+idu+'" value="'+codafectacion+'"> <input type="hidden" id="codtributo'+idu+'" name="codtributo'+idu+'" value="'+codtributo+'"> <input type="hidden" id="porcigv'+idu+'" name="porcigv'+idu+'" value="'+porcigv+'"> <input type="hidden" id="valorunitref'+idu+'" name="valorunitref'+idu+'" value="'+valorunitarioref+'"> <input type="hidden" id="igvref'+idu+'" name="igvref'+idu+'" value="'+igvref+'"> <input type="hidden" id="valortotalref'+idu+'" name="valortotalref'+idu+'" value="'+valortotalref+'">';
...
carga_util=result[i].cell[21];
cantidad_toneladas=result[i].cell[22];
articulo = _descripcionConAfectacionVenta(articulo, tipoope);
			...
			success: function(datos){
				...
				if(datos.pasa=='NO'){
					...
					var igv='0.00';
						...
						var exonf='0.00';
						var gravadas='0.00';
						var codAfectacionSel=_tipoToAfectacionVenta(tipoigv);
						var tribSel=_tributoPorAfectacionVenta(codAfectacionSel);
						var valorunitarioref='0.000000';
						var igvref='0.00';
						var valortotalref='0.00';

					var articuloBase=$('#titdetalle'+id).val()+''+titserie;
...
//PUNTOS DE COMPRA	
						if(puntos!='0'){
							puntos=(parseFloat(puntos)*parseFloat(cti));
													...
													var tipoCalcDet=_tipoCalculoLegacyVenta(codAfectacionSel);
							if(tipoCalcDet=='2'){
								valor=precioini;
								tipoope=codAfectacionSel;
								gratuitas=subtotal;
								sub='0.00';
								exonf='0.00';
								igv='0.00';
								gravadas='0.00';
								subtotal='0.00';
								valorunitarioref=_moneyNum(precioini).toFixed(6);
								valortotalref=_moneyFix2(gratuitas);
								if(codAfectacionSel==='14' || codAfectacionSel==='15'){
									igvref=_moneyFix2(_moneyNum(valortotalref) * (_moneyNum(tribSel.porc_igv) / 100));
								}
							} else if(tipoCalcDet=='3'){
								valor=precioini;
								tipoope=codAfectacionSel;
								exonf='0.00';
								inafecta=subtotal;
						...
						var articulo = _descripcionConAfectacionVenta(articuloBase, codAfectacionSel);
						var campo='<input type="hidden" id="idp'+cont+'" name="idp'+cont+'" value="'+id+'"> <input type="hidden" id="tipoart'+cont+'" name="tipoart'+cont+'" value="0"> <input type="hidden" id="codigo'+cont+'" name="codigo'+cont+'" value="'+codigo+'"> <input type="hidden" id="umedida'+cont+'" name="umedida'+cont+'" value="'+umedida+'"> <input type="hidden" id="tit'+cont+'" name="tit'+cont+'" value="'+articulo+'"> <input type="hidden" id="cti2'+cont+'" name="cti2'+cont+'" value="'+ctimedida+'"> <input type="hidden" id="exon'+cont+'" name="exon'+cont+'" value="'+exonerado+'"> <input type="hidden" id="tipo'+cont+'" name="tipo'+cont+'" value="'+tipoope+'"> <input type="hidden" id="igvh'+cont+'" name="igvh'+cont+'" value="'+igv+'"> <input type="hidden" id="exo'+cont+'" name="exo'+cont+'" value="'+exonf+'">  <input type="hidden" id="gravadas'+cont+'" name="gravadas'+cont+'" value="'+gravadas+'"> <input type="hidden" id="comision'+cont+'" name="comision'+cont+'" value="'+comision+'"> <input type="hidden" id="idunid'+cont+'" name="idunit'+cont+'" value="'+idunit+'"> <input type="hidden" id="grat'+cont+'" name="grat'+cont+'" value="'+gratuitas+'"> <input type="hidden" id="precioo'+cont+'" name="precioo'+cont+'" value="'+_moneyNum(precioini).toFixed(6)+'"> <input type="hidden" id="serie'+cont+'" name="serie'+cont+'" value="'+idserie+'"> <input type="hidden" id="puntos'+cont+'" name="puntos'+cont+'" value="'+puntos+'"> <input type="hidden" id="ctipuntos'+cont+'" name="ctipuntos'+cont+'" value="'+ctipuntos+'">  <input type="hidden" id="totdetraccion'+cont+'" name="totdetraccion'+cont+'" value="'+totdetraccion+'">   <input type="hidden" id="cantidad_toneladas'+cont+'" name="cantidad_toneladas'+cont+'" value="0">  <input type="hidden" id="carga_util'+cont+'" name="carga_util'+cont+'" value="0">  <input type="hidden" id="iddestino'+cont+'" name="iddestino'+cont+'" value="0"> <input type="hidden" id="codafectacion'+cont+'" name="codafectacion'+cont+'" value="'+codAfectacionSel+'"> <input type="hidden" id="codtributo'+cont+'" name="codtributo'+cont+'" value="'+tribSel.cod_tributo+'"> <input type="hidden" id="porcigv'+cont+'" name="porcigv'+cont+'" value="'+tribSel.porc_igv+'"> <input type="hidden" id="valorunitref'+cont+'" name="valorunitref'+cont+'" value="'+valorunitarioref+'"> <input type="hidden" id="igvref'+cont+'" name="igvref'+cont+'" value="'+igvref+'"> <input type="hidden" id="valortotalref'+cont+'" name="valortotalref'+cont+'" value="'+valortotalref+'"> ';
...
function Buscarserv(){
if(!_validarClienteSeleccion()){ return false; }
...
var igvsub='0';
var codAfectacionServicio = _tipoToAfectacionVenta(tipoigv);
...
if(codAfectacionServicio==='10'){
if(calculaigv=='0'){
precio=precio;	
}else{
var tribSrvDoc = _tributoPorAfectacionVenta(codAfectacionServicio);
var tasaSrvDoc = _moneyNum(tribSrvDoc.porc_igv) / 100;
igvsub=parseFloat(precio*tasaSrvDoc);
...
var codAfectacionServ = _tipoToAfectacionVenta(tipoigv);
var esNoOnerosaServ = _esAfectacionNoOnerosaVenta(codAfectacionServ);

if(esNoOnerosaServ){
gratuitas=_moneyFix2(parseFloat(precio)*parseFloat(cti));
subtotal='0.00';
exonf='0.00';
gravadas='0.00';
igv='0.00';
...
var tribSrvSel = _tributoPorAfectacionVenta(codAfectacionServ);
var factorSrv = 1 + (_moneyNum(tribSrvSel.porc_igv) / 100);
if(!isFinite(factorSrv) || factorSrv <= 0){ factorSrv = 1.18; }
gravadas=parseFloat(precio/factorSrv);
igv=precio-gravadas;
igv=igv.toFixed(2);
...
var total = esNoOnerosaServ ? 0 : (parseFloat(precio)*parseFloat(cti));
...
var articuloTitulo = _descripcionConAfectacionVenta('<b>'+titservicio+'</b>', codAfectacionServ);
var articulo=articuloTitulo+'\n'+detalleserv;
	
var tribServ = _tributoPorAfectacionVenta(codAfectacionServ);
var valorUnitRefServ = _esAfectacionNoOnerosaVenta(codAfectacionServ) ? _moneyNum(precio).toFixed(6) : '0.000000';
var valorTotalRefServ = _esAfectacionNoOnerosaVenta(codAfectacionServ) ? _moneyFix2(parseFloat(precio)*parseFloat(cti)) : '0.00';
var igvRefServ = ((codAfectacionServ==='14' || codAfectacionServ==='15') ? _moneyFix2(_moneyNum(valorTotalRefServ) * (_moneyNum(tribServ.porc_igv) / 100)) : '0.00');
var campo='<input type="hidden" id="idp'+cont+'" name="idp'+cont+'" value="'+idu+'"> <input type="hidden" id="tipoart'+cont+'" name="tipoart'+cont+'" value="1"> <input type="hidden" id="codigo'+cont+'" name="codigo'+cont+'" value="'+codigo+'"> <input type="hidden" id="umedida'+cont+'" name="umedida'+cont+'" value="'+umedida+'"> <input type="hidden" id="tit'+cont+'" name="tit'+cont+'" value="'+articulo+'"> <input type="hidden" id="cti2'+cont+'" name="cti2'+cont+'" value="'+ctimedida+'"> <input type="hidden" id="exon'+cont+'" name="exon'+cont+'" value="'+exonf+'"> <input type="hidden" id="tipo'+cont+'" name="tipo'+cont+'" value="'+codAfectacionServ+'"> <input type="hidden" id="igvh'+cont+'" name="igvh'+cont+'" value="'+igv+'"> <input type="hidden" id="exo'+cont+'" name="exo'+cont+'" value="'+exonf+'"> <input type="hidden" id="sub'+cont+'" name="sub'+cont+'" value="'+subtotal+'"> <input type="hidden" id="gravadas'+cont+'" name="gravadas'+cont+'" value="'+gravadas+'"> <input type="hidden" id="comision'+cont+'" name="comision'+cont+'" value="'+comision+'"> <input type="hidden" id="idu'+id+'" name="idu'+id+'" value="'+cont+'"> <input type="hidden" id="idunid'+cont+'" name="idunit'+cont+'" value="'+idunit+'"> <input type="hidden" id="grat'+cont+'" name="grat'+cont+'" value="'+gratuitas+'"> <input type="hidden" id="precioo'+cont+'" name="precioo'+cont+'" value="'+_moneyNum(precio).toFixed(6)+'"> <input type="hidden" id="serie'+cont+'" name="serie'+cont+'" value="'+idserie+'"> <input type="hidden" id="puntos'+cont+'" name="puntos'+cont+'" value="0"> <input type="hidden" id="ctipuntos'+cont+'" name="ctipuntos'+cont+'" value="0">  <input type="hidden" id="totdetraccion'+cont+'" name="totdetraccion'+cont+'" value="'+totdetraccion2+'">   <input type="hidden" id="cantidad_toneladas'+cont+'" name="cantidad_toneladas'+cont+'" value="'+cantidad_toneladas+'">  <input type="hidden" id="carga_util'+cont+'" name="carga_util'+cont+'" value="'+carga_util+'">  <input type="hidden" id="iddestino'+cont+'" name="iddestino'+cont+'" value="'+destinoid2+'"> <input type="hidden" id="codafectacion'+cont+'" name="codafectacion'+cont+'" value="'+codAfectacionServ+'"> <input type="hidden" id="codtributo'+cont+'" name="codtributo'+cont+'" value="'+tribServ.cod_tributo+'"> <input type="hidden" id="porcigv'+cont+'" name="porcigv'+cont+'" value="'+tribServ.porc_igv+'"> <input type="hidden" id="valorunitref'+cont+'" name="valorunitref'+cont+'" value="'+valorUnitRefServ+'"> <input type="hidden" id="igvref'+cont+'" name="igvref'+cont+'" value="'+igvRefServ+'"> <input type="hidden" id="valortotalref'+cont+'" name="valortotalref'+cont+'" value="'+valorTotalRefServ+'">';
	
var ctif=' <input type="text" onkeyup="teclea('+cont+', '+precio+')" size="3" id="ctif'+cont+'" name="ctif'+cont+'" value="'+cti+'"> ';
	
var preciof=' <input type="text" onkeyup="teclea('+cont+', '+precio+')" size="5" id="preciof'+cont+'" name="preciof'+cont+'" value="'+precio+'"> ';
	
var totf=' <input type="text" size="5" id="totf'+cont+'" name="totf'+cont+'" value="'+_moneyFix2(total)+'" readonly > ';
...
formData.append('txtMONEDA', $('#txtMONEDA').val());
formData.append('txtCOD_MONEDA', $('#txtMONEDA').val());
formData.append('txtID_MONEDA', $('#txtMONEDA').val());
...
formData.append('dto_global_tipo', $('#dto_global_tipo').val()||'MONTO');
formData.append('dto_global_valor', ($('#dto_global_tipo').val()==='PCT' ? _moneyNum($('#dto_global_valor').val()).toFixed(4) : _moneyNum($('#descuentotot').val()).toFixed(4)));
formData.append('dto_global_monto_base', _moneyFix2($('#dto_global_monto_base').val()));
formData.append('dto_global_monto_igv', _moneyFix2($('#dto_global_monto_igv').val()));
formData.append('dto_global_aplica_antes_igv', $('#dto_global_aplica_antes_igv').val()||'1');
...
detalle["txtIMPORTE_DET"]=_moneyRaw($('#totf'+idp).val());
detalle["txtCANTIDAD_DET"]=_moneyRaw($('#ctif'+idp).val());
detalle["txtPRECIO"]=_moneyRaw($('#preciof'+idp).val());
sumtotal=detalle["txtIMPORTE_DET"];
exonerada=$('#tipo'+idp).val();
var codAfectacionDet = ($('#codafectacion'+idp).length ? $('#codafectacion'+idp).val() : _tipoToAfectacionVenta(exonerada));
codAfectacionDet = _tipoToAfectacionVenta(codAfectacionDet);
var tipoLegacyDet = _tipoCalculoLegacyVenta(codAfectacionDet);
detalle["tipo"]=tipoLegacyDet;
detalle["tipounidad"]=tipoLegacyDet;	
detalle["txtPRECIO_DET"]=_moneyRaw($('#sub'+idp).val());
detalle["txtIGV"]=_moneyRaw($('#igv'+idp).val());
detalle["txtDESCRIPCION_DET"]=$('#tit'+idp).val();
detalle["UNIDAD_MEDIDA"]=$('#umedida'+idp).val();

detalle["txtCODIGO_DET"]=$('#codigo'+idp).val();
detalle["ctiunidad"]=_moneyRaw($('#cti2'+idp).val());
detalle["comision"]=_moneyRaw($('#comision'+idp).val());
detalle["idunit"]=$('#idunid'+idp).val();
/*
console.log('unidad:'+$('#umedida'+idp).val());
console.log('idunidad:'+$('#idunid'+idp).val());
*/
detalle["exonerado"]=_moneyRaw($('#exo'+idp).val());
detalle["gratuita"]=_moneyRaw($('#grat'+idp).val());
detalle["tipoart"]=$('#tipoart'+idp).val();		
detalle["serie"]=$('#serie'+idp).val();
detalle["descuento"]=_moneyRaw($('#descuento'+idp).val());
detalle["placa"]='';

detalle["detracciond"]=_moneyRaw($('#totdetraccion'+idp).val());
detalle["iddestino"]=$('#iddestino'+idp).val();
detalle["carga_util"]=_moneyRaw($('#carga_util'+idp).val());
detalle["cantidad_toneladas"]=_moneyRaw($('#cantidad_toneladas'+idp).val());
detalle["inafecta"]=_moneyRaw($('#inafecta'+idp).val());
detalle["guia5"]=$('#fpago_mpago').val();
var tribDet = _tributoPorAfectacionVenta(codAfectacionDet);
detalle["cod_afectacion_igv"]=codAfectacionDet;
detalle["cod_tributo"]=$('#codtributo'+idp).length ? $('#codtributo'+idp).val() : tribDet.cod_tributo;
detalle["porc_igv"]=$('#porcigv'+idp).length ? $('#porcigv'+idp).val() : tribDet.porc_igv;
var _vuRef = $('#valorunitref'+idp).length ? _moneyRaw($('#valorunitref'+idp).val()) : _moneyRaw($('#precioo'+idp).val()||$('#preciof'+idp).val());
if(['11','12','13','14','15','16','21','31','32','33','34','35','36'].indexOf(codAfectacionDet)<0){ _vuRef='0.000000'; }
detalle["valor_unitario_ref"]=_vuRef;
detalle["igv_ref"]=$('#igvref'+idp).length ? _moneyRaw($('#igvref'+idp).val()) : '0.00';
detalle["valor_total_ref"]=$('#valortotalref'+idp).length ? _moneyRaw($('#valortotalref'+idp).val()) : '0.00';
detalle["idcatalogo_afectacion"]=tribDet.idcatalogo_afectacion;
detalle["codigo_tipo_precio"]=tribDet.codigo_tipo_precio;
detalle["es_gratuito"]=tribDet.es_gratuito;
detalle["requiere_valor_referencial"]=tribDet.requiere_valor_referencial;
});
DATA.push(detalle);
}	
num=num+1;		
...
if(DATA.length===0){
	try{
		$("#detpedidos input[id^='idp']").each(function(){
			...
			detalle["txtCANTIDAD_DET"]=_moneyRaw($('#ctif'+idp).val());
			detalle["txtPRECIO"]=_moneyRaw($('#preciof'+idp).val());
			var codAfectacionDet = ($('#codafectacion'+idp).length ? $('#codafectacion'+idp).val() : _tipoToAfectacionVenta($('#tipo'+idp).val()));
			codAfectacionDet = _tipoToAfectacionVenta(codAfectacionDet);
			var tipoLegacyDet = _tipoCalculoLegacyVenta(codAfectacionDet);
			detalle["tipo"]=tipoLegacyDet;
			detalle["tipounidad"]=tipoLegacyDet;
...
detalle["es_gratuito"]=tribDet.es_gratuito;
detalle["requiere_valor_referencial"]=tribDet.requiere_valor_referencial;
			if(detalle["txtID"]){ DATA.push(detalle); }
		});
...
if(DATA2.length>0){
	var totalFacturaCuotas = _obtenerTotalCuotasVenta();
	var sumaCuotas = 0;
...
$.ajax({
	    ...
	    processData: false,
			...
			console.log(datos);
			var mensajeRespuesta = (datos && datos.mensaje) ? datos.mensaje : 'Proceso realizado';
			var guardadoOk = (datos && String(datos.idventa||'0') !== '0');
			...
			if(datos.estado=='1' || String(directosunat)==='1'){
				$('.guardaventa').prop('disabled', true);
				$('#agregar2,#agregars2,#guardar2,#guardarenv2').prop('disabled', true);
			}else{
				$('.guardaventa').prop('disabled', false);
				$('#agregar2,#agregars2,#guardar2,#guardarenv2').prop('disabled', false);
			}
			...
			if(guardadoOk){
				$('#nuevaventa2').prop('disabled', false);
				if(String(directosunat)!=='1' && datos.estado!='1'){
					$('.guardaventa').prop('disabled', false);
					$('#agregar2,#agregars2,#guardar2,#guardarenv2').prop('disabled', false);
				}
				listar();
				Swal.fire(mensajeRespuesta);
			}else{
				Swal.fire(mensajeRespuesta);
			}
...
// ======================================================
// CUOTAS (CRÉDITO): Base para cuotas
// Regla: TOTAL VENTA - DETRACCIÓN - RETENCIÓN
// Fix: Antes se usaba #txtTOTAL (total bruto), lo que generaba
//      cuotas incorrectas cuando existía detracción/retención.
// ======================================================
var baseCuotas = _obtenerTotalCuotasVenta();
$('#montopago2').val(_moneyFix2(baseCuotas));
$('#saldo2').val(_moneyFix2(baseCuotas));
