...
echo $rspta ? "Venta registrada" : "No se pudieron registrar todos los datos de la venta";
	...
	case 'mostrarpedido':
$sql="SELECT v.idventa, v.txtOBSERVACION, DATE(v.txtFECHA_DOCUMENTO) as fecha, v.txtID_CLIENTE, p.nombre as cliente, u.idusuario, u.nombre as usuario, v.txtID_TIPO_DOCUMENTO, v.txtSERIE, v.txtNUMERO, v.txtTOTAL, v.txtIGV, v.estado, v.tipoguia, v.guia, v.tipoguia2, v.guia2, v.tipoguia3, v.guia3, v.tipoguia4, v.guia4, v.tipoguia5, v.guia5, v.fpago_mpago,  v.presupuesto, v.referencia, v.doc_relaciona, v.txtID_MONEDA, v.tipo_pago, v.medio_pago, v.orden, v.sector, v.iddetraccion, v.detraccion, dt.codigo AS codidetracciones, dt.porcentaje, v.retencion, v.percepcion, v.referencial FROM venta v INNER JOIN persona p ON v.txtID_CLIENTE=p.idpersona INNER JOIN usuario u ON v.idusuario=u.idusuario LEFT JOIN detracciones dt ON dt.id=v.iddetraccion WHERE v.idventa='$idventa' ";
...
$tipoigv='<select id="tipoigv'.$reg->txtCOD_ARTICULO.'" name="tipoigv'.$reg->txtCOD_ARTICULO.'" class="form-control" style="width: 70px; padding: 1px; margin: 1px;  height: 25px; " />
<option value="0" ';

if($reg->exonerado_igv=='0'){ $tipoigv.='selected'; }

$tipoigv.=' >IGV 18%</option>
<option  value="1"  ';
	
if($reg->exonerado_igv=='1'){ $tipoigv.='selected'; }
	
$tipoigv.=' >EXONERADA</option>';

$tipoigv.='<option  value="3"  ';	
if($reg->exonerado_igv=='3'){ $tipoigv.='selected'; }
$tipoigv.=' >INAFECTA</option>';
	
if($reg->canje=='SI'){
if($cli['puntos']>=$reg->canjepuntos){	
$tipoigv.='<option selected value="2">GRATUITA</option>';	
}
}else{
$tipoigv.='<option value="2">GRATUITA</option>';		
}
$tipoigv.='</select>';
...
$tipoigv='<select id="tipoigv'.$reg->txtCOD_ARTICULO.'" name="tipoigv'.$reg->txtCOD_ARTICULO.'" class="form-control" style="width: 70px; padding: 1px; margin: 1px;  height: 25px; " />
	...
	if($exoneradoigv=='0'){ $tipoigv.='selected'; }
	
	$tipoigv.=' >IGV 18%</option>
	<option  value="1"  ';
		
	if($exoneradoigv=='1'){ $tipoigv.='selected'; }
		
	$tipoigv.=' >EXONERADA</option>';
	
	$tipoigv.='<option  value="3"  ';	
	if($exoneradoigv=='3'){ $tipoigv.='selected'; }
	$tipoigv.=' >INAFECTA</option>';
...
$preciosinigv=round(($preciofinal/1.18), 5);
...
(pedido, sector, txtID_CLIENTE, idusuario, idlocal, doc_relaciona, txtID_TIPO_DOCUMENTO, txtSERIE, txtNUMERO, txtFECHA_DOCUMENTO, txtOBSERVACION, txtSUB_TOTAL, txtIGV, txtTOTAL, gratuita, exonerado, comision, txtID_MONEDA, tipo_pago, estado, docmodifica_tipo, docmodifica, modifica_motivo, modifica_motivod, hash_cpe, hash_cdr, mensaje)
	*/
        ...
        $sql = "INSERT INTO venta (idventa, idempresa, beta, pedido, sector, txtID_CLIENTE, idusuario, idlocal, txtID_TIPO_DOCUMENTO, doc_relaciona, docmodifica_tipo, docmodifica, modifica_motivo, modifica_motivod, txtSERIE, txtNUMERO, txtFECHA_DOCUMENTO, fecha_vto, txtOBSERVACION, txtSUB_TOTAL, txtIGV, ICB, descuento, txtTOTAL, gratuita, exonerado, comision, tarjeta, txtID_MONEDA, tipo_pago, medio_pago, orden, tipoguia, guia, presupuesto, referencia, condiciones, hash_cpe, hash_cdr, mensaje, estado, estadopago, retencion, iddetraccion, detraccion, tipocambio,inafecta, percepcion, kardex, referencial, controlpresupuestal, tipoguia2, guia2, tipoguia3, guia3, tipoguia4, guia4, tipoguia5, guia5, idcaja, exportacion, fpago_mpago)

SELECT NULL, idempresa, beta, '$idventa', sector, txtID_CLIENTE, idusuario, idlocal, '$tdoc', txtID_TIPO_DOCUMENTO, docmodifica_tipo, docmodifica, modifica_motivo, modifica_motivod, '$seriedoc', '$num', NOW(), NOW(), txtOBSERVACION, txtSUB_TOTAL, txtIGV, ICB, descuento, txtTOTAL, gratuita, exonerado, comision, tarjeta, txtID_MONEDA, tipo_pago, medio_pago, orden, tipoguia, guia, presupuesto, '$docrelaciona', condiciones, hash_cpe, hash_cdr, mensaje, '0', estadopago, retencion, iddetraccion, detraccion, tipocambio, inafecta, percepcion, '0', referencial, controlpresupuestal, tipoguia2, guia2, tipoguia3, guia3, tipoguia4, guia4, tipoguia5, guia5, idcaja, exportacion, COALESCE(fpago_mpago, guia5) FROM venta WHERE idventa='$idventa' ";
...
$rspta = $resumen->detfactura($idventa);
	*/
        ...
        while ($reg = $rspta->fetch_object()) {
            ...
            $sql_detalle = "INSERT INTO detalle_venta VALUES (NULL, '$_COOKIE[id]', '$_COOKIE[idlocal]', '$reg->beta', '$idventanew', '$reg->tipoarticulo', '$reg->idproducto', '$reg->codigoproducto', '$reg->unidadmedida', '$reg->idpresentacion', '$reg->nombreproducto', '$reg->txtCANTIDAD_ARTICULO', '$reg->cantidadp', '$reg->precio', '$reg->preciocompra', '$reg->descuento', '$reg->subtotal', '$reg->igv', '$reg->ICB', '$reg->importe', '$reg->exoneradod', '$reg->inafectad', '$reg->gratuitad', '$reg->detracciond',  '$reg->comisiond', '$reg->stock', '$reg->tipo', '$reg->anticipo',  '$reg->doc_anticipo', '$reg->placa',  '$reg->idlote', '$reg->idproveedor', '$reg->iddestino', '$reg->carga_util', '$reg->cantidad_toneladas', '$reg->fecha', '$reg->idcaja')";
...
if($nivel=='0'){
	...
	$totalfinal=$mos2['txtTOTAL']-$mos2['detraccion']-$mos2['retencion']+$mos2['percepcion']+$detraccionpagado+$retencionpagado;
