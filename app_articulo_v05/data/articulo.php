...
$precio_mayor3=isset($_POST["precio_mayor3"])? limpiarCadena($_POST["precio_mayor3"]):"0";
...
$maneja_lote=isset($_POST["maneja_lote"])? limpiarCadena($_POST["maneja_lote"]):"0";
$maneja_serie=isset($_POST["maneja_serie"])? limpiarCadena($_POST["maneja_serie"]):"0";
$maneja_garantia=isset($_POST["maneja_garantia"])? limpiarCadena($_POST["maneja_garantia"]):"0";
$garantia_tipo=isset($_POST["garantia_tipo"])? limpiarCadena($_POST["garantia_tipo"]):"NINGUNA";
$garantia_meses=isset($_POST["garantia_meses"])? limpiarCadena($_POST["garantia_meses"]):"0";
$idcatalogo_afectacion=isset($_POST["idcatalogo_afectacion"])? limpiarCadena($_POST["idcatalogo_afectacion"]):"";
...
case 'guardaryeditar':
  ...
  $precio_mayor3 = isset($_POST['precio_mayor3']) ? trim((string)$_POST['precio_mayor3']) : '0';
  ...
  $maneja_lote = isset($_POST['maneja_lote']) ? (int)$_POST['maneja_lote'] : 0;
  $maneja_serie = isset($_POST['maneja_serie']) ? (int)$_POST['maneja_serie'] : 0;
  $maneja_garantia = isset($_POST['maneja_garantia']) ? (int)$_POST['maneja_garantia'] : 0;
  $garantia_tipo = isset($_POST['garantia_tipo']) ? trim((string)$_POST['garantia_tipo']) : 'NINGUNA';
  $garantia_meses = isset($_POST['garantia_meses']) ? (int)$_POST['garantia_meses'] : 0;
  $idcatalogo_afectacion = isset($_POST['idcatalogo_afectacion']) ? trim((string)$_POST['idcatalogo_afectacion']) : '';

  if ($garantia_meses < 0) { $garantia_meses = 0; }
  if ($maneja_garantia !== 1) {
    $garantia_tipo = 'NINGUNA';
    $garantia_meses = 0;
  } else {
    $tiposValidos = ['NINGUNA','FABRICANTE','COMERCIAL','EXTENDIDA'];
    if (!in_array($garantia_tipo, $tiposValidos, true)) { $garantia_tipo = 'NINGUNA'; }
  }
  ...
  if ($tipoac == '2') {
    ...
    $rspta = $articulo->insertar($cat, $precio_mayor2, $precio_porcentaje2, $precio_porcentaje3, $precio_mayor3, $precio);
    ...
    if ($codigo === '') { $codigo = (string)$txtCOD_ARTICULO; }
	    ...
	    $rspta = $articulo->editar(
	      ...
	      $canje, $canjepuntos, $canjecobro, $precio_mayor2, $precio_porcentaje2, $precio_porcentaje3, $precio_mayor3, $idcatalogo_afectacion,
	        $maneja_lote, $maneja_serie, $maneja_garantia, $garantia_tipo, $garantia_meses
		    );
...
case 'listar':
  ...
  $filtro_grupo   = isset($_POST['filtro_grupo']) ? trim((string)$_POST['filtro_grupo']) : '0';
  $filtro_marca   = isset($_POST['filtro_marca']) ? trim((string)$_POST['filtro_marca']) : '0';
  $filtro_linea   = isset($_POST['filtro_linea']) ? trim((string)$_POST['filtro_linea']) : '0';
  $filtro_stock   = isset($_POST['filtro_stock']) ? trim((string)$_POST['filtro_stock']) : '0';
  ...
  $whereFiltros = "";
  if ($filtro_grupo !== '' && $filtro_grupo !== '0') {
    $fg = str_replace(["\\", "'"], ["\\\\", "\\'"], $filtro_grupo);
    $whereFiltros .= " AND a.idcategoria='$fg' ";
  }
  if ($filtro_marca !== '' && $filtro_marca !== '0') {
    $fm = str_replace(["\\", "'"], ["\\\\", "\\'"], $filtro_marca);
    $whereFiltros .= " AND a.marca='$fm' ";
  }
  if ($filtro_linea !== '' && $filtro_linea !== '0') {
    $fl = str_replace(["\\", "'"], ["\\\\", "\\'"], $filtro_linea);
    $whereFiltros .= " AND a.linea='$fl' ";
  }
  if ($filtro_stock === '1') {
    $whereFiltros .= " AND IFNULL(s.stock,0) > 0 ";
  }
  ...
  $records = ejecutarConsultaSimpleFila("
    ...
    WHERE a.idempresa='$idempresa' $whereLocal $whereFiltros
  ");
  ...
  $records = ejecutarConsultaSimpleFila("
    ...
    LEFT JOIN articulo_stock s ON s.idarticulo=a.txtCOD_ARTICULO AND s.idlocal='$idlocal_cookie'
    WHERE a.idempresa='$idempresa' $whereLocal $whereFiltros $searchQuery
  ");
  ...
  // Query principal
  // =========================
  $colMoneda = "'PEN' AS moneda";
  $colMedida = "'' AS medida";
  $chkMoneda = ejecutarConsultaSimpleFila("SHOW COLUMNS FROM articulo LIKE 'moneda'");
  $chkMedida = ejecutarConsultaSimpleFila("SHOW COLUMNS FROM articulo LIKE 'medida'");
  if ($chkMoneda) { $colMoneda = "a.moneda AS moneda"; }
  if ($chkMedida) { $colMedida = "a.medida AS medida"; }
  ...
  while ($reg = $empRecords->fetch_object()) {
    ...
    $botones = '
      <div class="dropdown">
        <button class="btn btn-xs dropdown-toggle btn-opciones-art" style="background:#337ab7;color:#fff;border-color:#2e6da4;" type="button" aria-haspopup="true" aria-expanded="false">
    ...
    $stockf  = number_format((float)$reg->stock_local, 0, '.', ',');
    $precio  = number_format((float)$reg->precio, 4, '.', ',');
    $pcompra = number_format((float)$reg->precio_compra, 4, '.', ',');
    $presentaciones = [];
    $rsPres = ejecutarConsulta("SELECT precio, IFNULL(NULLIF(nombre,''), medida) AS nombre_pres FROM articulo_unidad WHERE idproducto='".$reg->txtCOD_ARTICULO."' ORDER BY id ASC");
    if ($rsPres){
      while($pr = $rsPres->fetch_object()){
        $presentaciones[] = number_format((float)$pr->precio, 4, '.', ',').' | '.htmlspecialchars($pr->nombre_pres);
      }
    }
    $titMedida = isset($reg->medida) ? $reg->medida : '';
    $um = ejecutarConsultaSimpleFila("SELECT tit FROM unidad_medida WHERE codigo='".str_replace("'", "\\'", $titMedida)."' LIMIT 1");
    if ($um && isset($um['tit']) && $um['tit']!=='') { $titMedida = $um['tit']; }
    $presentacionBase = $precio.' | '.htmlspecialchars($titMedida);
    $selPresent = '<select class="form-control input-sm sel-present-art">';
    $selPresent .= '<option selected data-precio="'.$precio.'" value="'.htmlspecialchars($presentacionBase).'">'.htmlspecialchars($presentacionBase).'</option>';
    foreach($presentaciones as $pTxt){
      $partes = explode(' | ', html_entity_decode($pTxt), 2);
      $precioPres = isset($partes[0]) ? trim($partes[0]) : $precio;
      $selPresent .= '<option data-precio="'.htmlspecialchars($precioPres).'" value="'.$pTxt.'">'.$pTxt.'</option>';
    }
    $selPresent .= '</select>';
    $precioHtml = '<span class="precio-art">'.$precio.'</span>';

    $badgeMoneda = '<span class="label" style="display:inline-block;min-width:42px;'
      .(($reg->moneda==='USD') ? 'background:#2ca02c;color:#fff;' : 'background:#f0ad4e;color:#222;')
      .'">'.(($reg->moneda==='USD') ? 'USD' : 'PEN').'</span>';

    $selLotes = '<select class="form-control input-sm sel-lote-art">';
    $sqlLotes = "SELECT lote, fecha_vto FROM articulo_serie WHERE cod_articulo='".$reg->txtCOD_ARTICULO."' ORDER BY id DESC";
    $rsLotes = ejecutarConsulta($sqlLotes);
    $fvDefault = '';
    $isFirstLote = true;
    if ($rsLotes){
      while($lt = $rsLotes->fetch_object()){
        if ($isFirstLote) {
          $fvDefault = ($lt->fecha_vto=='0000-00-00') ? '' : $lt->fecha_vto;
        }
        $fvDato = ($lt->fecha_vto=='0000-00-00') ? '' : $lt->fecha_vto;
        $selected = $isFirstLote ? ' selected' : '';
        $selLotes .= '<option data-fv="'.$fvDato.'" value="'.htmlspecialchars($lt->lote).'"'.$selected.'>'.htmlspecialchars($lt->lote).'</option>';
        $isFirstLote = false;
      }
    }
    if($selLotes === '<select class="form-control input-sm sel-lote-art">'){
      $selLotes .= '<option data-fv="" value="">--</option>';
    }
    $selLotes .= '</select>';
    if ($fvDefault==='') { $fvDefault = (string)$reg->fecha_vto; }
    ...
    $data[] = [
      ...
      $reg->lab4,                    // 11 hidden
      $selLotes,                     // 12
      '<span class="fv-lote-art">'.$fvDefault.'</span>', // 13
      $estado                        // 14
    ];
  }
...
case 'filtro_grupo':
  ...
  $idempresa = isset($_COOKIE['id']) ? $_COOKIE['id'] : '0';
  echo '<option value="0" selected>TODOS LOS GRUPOS</option>';
...
case 'filtro_marca':
  ...
  $rspta = ejecutarConsulta("
    SELECT DISTINCT a.marca, m.nombre
    FROM articulo a
    INNER JOIN categoria m ON m.idcategoria=a.marca
    WHERE a.idempresa='$idempresa' AND a.marca<>'0'
    ORDER BY m.nombre ASC
  ");
  while($reg = $rspta->fetch_object()){
    echo '<option value="'.$reg->marca.'">'.$reg->nombre.'</option>';
  }
...
case 'filtro_linea':
  ...
  $rspta = ejecutarConsulta("
    SELECT DISTINCT a.linea, l.nombre
    FROM articulo a
    INNER JOIN categoria l ON l.idcategoria=a.linea
    WHERE a.idempresa='$idempresa' AND a.linea<>'0'
    ORDER BY l.nombre ASC
  ");
  while($reg = $rspta->fetch_object()){
    echo '<option value="'.$reg->linea.'">'.$reg->nombre.'</option>';
  }
...
case 'tempproducto':
    ...
    if (isset($_COOKIE['idarticulo']) && $_COOKIE['idarticulo'] != '') {
        ...
        // Crear temporal
        $sql = "INSERT INTO tmp_articulo (
            id,idempresa,idcategoria,marca,linea,sublinea,subfamilia,medida,sanitario,idlocal,idproveedor,codigo,codigosunat,existencia,txtDESCRIPCION_ARTICULO,stock,stockmin,stockmax,precio,precio_compra,mayor,precio_mayor,exonerado_igv,comision,comisionm,comisionmp,bolsa,ctacompras,ctaventas,imagen,condicion,resaltado,maneja_lote,maneja_serie,maneja_garantia,garantia_tipo,garantia_meses
        ) VALUES (
            NULL,
            '$idempresa',
            '0','233','0','0','0','0','0',
...
if ((int)$garantia_meses < 0) { $garantia_meses = 0; }
if ((int)$maneja_garantia !== 1) {
  $garantia_tipo = 'NINGUNA';
  $garantia_meses = 0;
}
$rspta=$articulo->editartemp($idcategoria, $idmarca, $codigo, $txtDESCRIPCION_ARTICULO, $stock, $stockmin, $stockmax, $precio, $ctimayor, $pmayor, $medida, $exonerado, $comision, $comisionm, $comisionmp, $codigos, $idproveedor, $sanitario, $precioc, $linea, $sublinea, $subfamilia, $ctacompras, $ctaventas, $bolsa, $idlocal, $existencia, $precio_mayor3, $idcatalogo_afectacion, $maneja_lote, $maneja_serie, $maneja_garantia, $garantia_tipo, $garantia_meses);
